...
def Registry(path):  #레지스트리 내용 수집(SAM, SECURITY, SOFTWARE, SYSTEM, DEFAULT, NTUSER.DAT)
    try:
        ...
        check = os.path.isdir(str(registry_path))
        if check == False :
...
def continuous_execution(path) : # 지속실행 등록
    try:
        ...
        copy_tree(Startup_path, path+"/Desktop/Art/Startup") 
        copy_tree(Scheduler_path, path+"/Desktop/Art/Scheduler") 
        shutil.copy2(SystemTime_path+"\Security.evtx", path+"/Desktop/Art/System Time") 
        shutil.copy2(SystemTime_path+"\System.evtx", path+"/Desktop/Art/System Time")
...
def TimeLine(path) :
    try:
        ...
        check = os.path.isdir(str(timeline_path))
        if check == False :
            print("[!] Not Found Prefetcg Path.... Default Path is "+check)
        else :
...
def WER(path) : #WER(Windows Error Reporting) 수집
    try:
        ...
        check = os.path.isdir(str(WER_path))
        if check == False :
            print("[!] Not Found Prefetcg Path.... Default Path is "+check)
        else :
        ...
        copy_tree(WER_path, path+"/Desktop/Art/WER") # 폴더 통채로 복사 , shutil.copy2()는 파일 한개 복사
